...
public class LoyaltyPayment {
	 ...
	 private static final int LOYALTY_PAYMENT = 2;
	...
	public void start() throws Exception{
		 ...
		 if(input == 0){
			 ...
			 String result = payment.addPayment(clientName, (amount-discountedAmount), discountedAmount,LOYALTY_PAYMENT);
			 ...
			 if(resultSet!=null){
				 ...
				 Payment payment = new ProcessPayment();
				 String result = payment.updatePayment(input,totalAmount, totalDiscountedAmount,LOYALTY_PAYMENT);
				 if(result.equalsIgnoreCase("success")){
					 System.out.println("Amount added successfully !!!");	 
				 }else if(result.equalsIgnoreCase("error")){
					 System.out.println("Failed!!!");
	...
	private ResultSet isClientExist(int input) throws Exception {
		...
		return dao.searchClient(input,LOYALTY_PAYMENT,connection);
	...
	private void fetchExistingClient() throws SQLException {
		...
		try{
			...
			resultSet = dao.getExistingData(LOYALTY_PAYMENT, connection);
